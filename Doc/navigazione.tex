...
\subsection{Campi di potenziale}
	...
	Le forze esercitate dai campi di potenziale hanno per convenzione intensità nell'intervallo $[0,1]$. \\
	 \\
	Il campo \textbf{Wander}, che permette l'esplorazione dell'ambiente, applica al robot una forza costante in avanti rispetto al suo orientamento. 
	...
	\begin{figure}[h!]
		...
		\caption{Plot della funzione del campo Avoid.}
		\label{fig:repulsion_plot}
	\end{figure}
	\newpage
	Il campo \textbf{Avoid} permette la \textit{Obstacle Avoidance}. Ogni ostacolo genera un campo repulsivo con un profilo di ampiezza esponenziale. In particolare l'intensità della forza generata dal campo segue la funzione:
	\begin{center}
		$avoid(d) = b^{-c \cdot (d-D_S)} $
	\end{center}
	...
	\begin{figure}[h!]
		...
		\caption{Plot della funzione del campo Move to Goal.}
		\label{fig:attraction_plot}
	\end{figure}

	Il campo \textbf{Move to Goal} permette al robot di raggiungere un punto di interesse, che nel caso in esame è il colore a cui avvicinarsi. Il punto di interesse (POI) genera un campo attrattivo di intensità lineare sulla distanza dal robot che segue la funzione:
	\begin{center}
		$attract(d) = \frac{1-F_{min}}{D_S-D_A}\cdot(d-D_S)+1$
	\end{center}
...
\subsection{Orientamento}
	...
	Ottenere questi dati è possibile grazie all'utilizzo dell'interfaccia RosAria per la gestione della mobilità del robot. Il nodo ROS RosAria pubblica sul topic \texttt {/RosAria/pose} le informazioni sulla posizione e sull'orientamento del robot. I messaggi pubblicati sul topic utilizzano un sistema di coordinate mondocentrico, i cui orientamento e origine coincidono con l'orientamento e la posizione del robot al momento dell'avvio del nodo RosAria.
